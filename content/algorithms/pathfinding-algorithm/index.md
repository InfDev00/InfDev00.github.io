...
A*는 그 자체로도 강력하지만, 상황에 따라 비효율적이게 되는 경우가 있다. 이를 보완하는 변형 알고리즘들을 소개한다.

### JPS — 대칭 경로 제거

장애물 없는 넓은 공간에서 두 지점을 잇는 같은 비용의 경로는 수없이 많다. A*는 이 경로들을 구별하지 못하고 모두 Open list에 올린다. 맵이 넓을수록 이 낭비가 커진다.

**JPS(Jump Point Search)** 는 이 문제를 해결한다. 한 방향으로 이동하는 중에 주위에 장애물을 만나지 않는다면 직진이 최단 거리다.
따라서 JPS는 장애물로 인해 경로가 갈라지는 **점프 포인트**까지 한 번에 건너뛴다. 이 지점에서만 Open list에 추가하므로 탐색 노드 수는 크게 줄어들고 최적해는 그대로 보장된다.
...
**Flow Field**는 접근 방식이 다르다. 목적지 하나에 대해 맵 전체의 이동 방향을 **한 번만** 계산하고, 각 셀에 벡터로 저장해둔다.

계산은 두 단계계로 이뤄진다. 먼저 목적지에서 역방향 BFS로 각 셀까지의 거리를 기록한다. 그 거리를 바탕으로 각 셀에 "어느 방향이 목적지에 가장 가까운가"를 벡터로 설정한다.

이후 에이전트는 자기 위치의 벡터만 읽어 이동한다. 탐색 비용이 에이전트 수와 무관하게 고정된다. 단, 목적지가 자주 바뀌면 매번 재계산해야 하므로 효과가 반감된다.
