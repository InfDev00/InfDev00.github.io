...
공식 가이드를 따라 하면 코드는 완성된다. 다만 `builder`와 `app`이 왜 나뉘는지, DI에서 매개변수가 어디서 오는지를 이해하지 못하면 코드를 수정하거나 확장할 때 막힌다.

Unity 배경이 있으면 오히려 유리한 부분이 있다. `async/await` 문법이 낯설지 않고, 컴포넌트 기반 사고가 DI 개념을 이해하는 데 도움이 된다.