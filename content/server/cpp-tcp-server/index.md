...
이전 과정에서는 asp.net을 활용해서 서버를 구축했다. 이번에는 프레임워크 없이 직접 만들어 보기로 했다. 언어는 C++을 골랐다. 운영체제가 제공하는 소켓 API를 추상화 없이 그대로 호출할 수 있고, 파일 디스크립터나 바이트 버퍼 같은 저수준 개념을 피하지 않고 마주하게 되기 때문이다.

목표는 **에코(echo) 서버**다. 클라이언트가 보낸 데이터를 그대로 되돌려주는, 기능이라곤 그것뿐인 서버다. 기능이 단순한 만큼 연결을 맺고 데이터를 주고받고 끊는 흐름 자체에만 집중할 수 있다.
...
이 서버에는 한 가지 한계가 있다. `handle_client`가 끝나야 `run`의 루프가 다음 `accept`로 돌아가기 때문에, 한 번에 클라이언트 **하나**만 처리한다.  
이를 해결하기 위해서는 쓰레드 혹은 이벤트 기반 방식을 고려해야 하겠다.
